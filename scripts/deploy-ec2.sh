...
# Webhook helper — sends progress to Go server (which forwards to frontend)
notify() {
  local type=$1 msg=$2
...
notify "deploying_app" "deploying OpenClaw to ${EC2_IP}"
...
notify "deploying_app" "copying files to EC2"
...
notify "pulling_image" "pulling Docker image on EC2"
...
notify "starting_app" "starting OpenClaw container"
...
notify "health_check" "checking container health"
ssh -T -i "$SSH_KEY" -o StrictHostKeyChecking=no ec2-user@${EC2_IP} << 'ENDSSH'
cd /home/ec2-user/openclaw
docker compose ps
...
if [ "${SETUP_CLOUDFLARE:-false}" = "true" ]; then
  ...
  notify "setting_up_cloudflare" "setting up Cloudflare tunnel"
  ...
  if [ -n "$TUNNEL_URL" ]; then
    ...
    notify "cloudflare_ready" "${TUNNEL_URL}/chat?session=main&token=${TOKEN}"
  else
...
echo -e "Logs: make ec2-logs NAME=${DEPLOY_NAME:-main}"
echo -e "Restart: make ec2-restart NAME=${DEPLOY_NAME:-main}"
echo -e "Approve: make ec2-approve NAME=${DEPLOY_NAME:-main}"
echo ""
...
if [ "${SKIP_APPROVAL:-false}" != "true" ]; then
  ...
  notify "auto_approving" "installing auto-approve watcher"
